...
export default function Home({ allNftCollections }: Home) {
  return (
    ...
    >
      {/* Heading */}
      <Heading textAlign="center" size="4xl" marginTop="8">
        UMOID MARKETPLACE
      </Heading>
      <Text textAlign="center" marginTop="2">
        Your one-stop marketplace to{" "}
        <HighlightedText>
          mint, list, buy and sell NFT collections
        </HighlightedText>
        .
      </Text>
      ...
      >
        {allNftCollections.slice(0, 3).map((nftCollection, index) => (
          ...
          >
            <Link
              passHref
              href={`/collections/${nftCollection.nftContractAddr}`}
            >
              <a>
                <Image
                  alt={`${nftCollection.name} NFT Collection - ${nftCollection.description}`}
                  src={getIpfsFileUri(
                    decodeMetadataUri(
                      nftCollection.nftsInCollection[0].metadataUri
                    ).image as string
                  )}
                  width="full"
                />
      ...
      <Heading textAlign="center" marginTop="8">
        These are just some NFT collections created here.{" "}
        <HighlightedText usedFor="heading">Create yours today!</HighlightedText>
      </Heading>
