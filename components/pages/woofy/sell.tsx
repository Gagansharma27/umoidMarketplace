                ...
                {progressFetchWoofys ?
                    ...
                    woofysToDisplay.map((woofyToDisplay) => (
                        <Box key={woofyToDisplay.tokenId.toString()} width="full" position="relative">
                            <Tooltip aria-label={type === "not-sell" ? `Previous listed price was ${ethers.utils.formatEther(woofyToDisplay.price)} BNB` : `Currently listed for ${ethers.utils.formatEther(woofyToDisplay.price)} BNB`} label={type === "not-sell" ? "Previous listed price" : "Currently listed for"} placement="top">
                                <Text textAlign="center" userSelect="none">{ethers.utils.formatEther(woofyToDisplay.price)} BNB</Text>
                            </Tooltip>
                ...
                <AlertDialogOverlay>
                    <AlertDialogContent>
                        ...
                        <AlertDialogHeader fontSize='lg' fontWeight='bold'>
                            {type === "not-sell" ? "Put NFT for sale" : "De-list NFT from sale"}
                        </AlertDialogHeader>
                        ...
                        <AlertDialogBody>
                            ...
                            {type === "not-sell" &&
                                <FormControl>
                                    ...
                                    <Input focusBorderColor="brand.500" value={sellAmount} onChange={(e) => { setSellAmount(e.target.value) }} type="number" placeholder="XX.XX (BNB)" />
                                    <FormHelperText>Set the price <i>(in BNB)</i> you want to sell this NFT for.</FormHelperText>
                                </FormControl>
                            }
                            {!!woofySelected &&
                                <Text fontStyle="italic" marginTop={type === "not-sell" ? "2" : "0"} fontSize="sm" fontWeight="600">{type === "not-sell" ? "Previously set" : "Listed for"}: {ethers.utils.formatEther(woofySelected?.price as BigNumber)} BNB</Text>
                            }
                        </AlertDialogBody>
